...
const isExists = (id) => {
    let temp = productList.filter((item) => item.product_id == id.product_id)
    return temp.length > 0 ? true : false
}

const handleCart = (ele) => {

    productList.push({...ele,qty:1})
    productList.push(ele.product_id)
    console.log(ele.product_id);
    if(isExists(ele.product_id)){
        productList.map((item,i)=>{
            if(item.product_id == ele.product_id){
                productList[i].qty += 1
            }
        })
        alert("Quentity Added Successfully")
    }
    else{
        productList.push({...ele,qty:1})
        alert("added to cart")
